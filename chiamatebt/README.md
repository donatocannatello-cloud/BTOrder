...
1. Apri la cartella **`chiamatebt/`** (o la root del repository) con
...
3. Da terminale, in alternativa (dalla root del repository, che ora ospita
   anche il modulo indipendente `ripassofoto/`):
   ```bash
   ./gradlew :chiamatebt:assembleDebug
   ```
   L'APK di debug viene generato in `chiamatebt/build/outputs/apk/debug/`.
